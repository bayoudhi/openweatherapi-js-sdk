const { getForecastByCityId } = require("../../../src/forecast-api/factories");

it('should reject "cityId is missing", when cityId is not passsed', async () => {
  const expectedResult = new Error("cityId is missing");
  await expect(getForecastByCityId({}, {})).rejects.toEqual(expectedResult);
  await expect(getForecastByCityId({}, { cityId: "" })).rejects.toEqual(
    expectedResult
  );
});

describe("getForecastByCityId({axios,endpoint,apiKey},{cityId})", () => {
      ...
      describe("when apiKey equals key007", () => {
        ...
        describe("when axios resolves", () => {
          ...
          describe("getForecastByCityId({axios,endpoint,apiKey},{cityId})", () => {
            ...
            beforeEach(async () => {
              try {
                result = await getForecastByCityId(
                  { axios, endpoint, apiKey },
                  { cityId }
                );
              } catch (e) {}
...
describe("getForecastByCityId({axios,endpoint,apiKey},{cityId,lang})", () => {
  describe("when cityId equals 2172797", () => {
    const cityId = "2172797";
    const lang = "ar";
    describe('when endpoint equals "https://api.openweathermap.org/data/2.5/forecast"', () => {
...
describe("getForecastByCityId({axios,endpoint,apiKey},{cityId,units})", () => {
  describe("when cityId equals 2172797", () => {
    const cityId = "2172797";
...
describe("getForecastByCityId({axios,endpoint,apiKey},{cityId,lang,units})", () => {
  describe("when cityId equals 2172797", () => {
    const cityId = "2172797";
    const lang = "ar";
    const units = "metric";
